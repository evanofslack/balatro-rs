...
fn render_main(f: &mut Frame, app: &mut AppState, area: Rect) {
    ...
    let spectral = match &app.game.stage {
        Stage::SpectralHand(s) => *s,
        _ => return,
    };
    ...
    render_spectral_info(f, &spectral, app, chunks[3]);
...
fn render_cards(f: &mut Frame, app: &mut AppState, area: Rect) {
    ...
    let cursor = if matches!(app.focus, FocusZone::SpectralCards) {
        app.cursor
    } else {
        usize::MAX
    };
...
fn render_apply_button(f: &mut Frame, app: &mut AppState, area: Rect) {
    ...
    let (needed, max) = match &app.game.stage {
        Stage::SpectralHand(s) => (s.min_targets(), s.max_targets()),
        _ => (0, usize::MAX),
    };
    ...
    let focused = matches!(app.focus, FocusZone::SpectralButtons);
    ...
    let para = Paragraph::new(Line::from(Span::styled("Apply Spectral", text_style)))
        .block(block)
        .alignment(Alignment::Center);
    f.render_widget(para, btn_rect);
    app.widget_rects
        .insert(WidgetId::SpectralButton(0), btn_rect);
}

fn render_spectral_info(
    f: &mut Frame,
    spectral: &balatro_rs::spectral::Spectral,
    app: &AppState,
    area: Rect,
) {
    let selected_count = app.game.available.selected().len();
    let min = spectral.min_targets();
    let max = spectral.max_targets();
    ...
    let lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(spectral.description(), Style::default().fg(Color::White)),
        ]),
        Line::from(""),
